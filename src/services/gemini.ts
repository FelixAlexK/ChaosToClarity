...
export async function sendBrainDumpToGemini(
	content: string,
): Promise<ResponseV2> {
	...
	if (!response || typeof response !== "object") {
		throw new Error(
			`Invalid response from AI (not an object). request=${JSON.stringify(payloadSummary)}`,
		);
	}
	...
	const parsedResponse = responseSchemaV2.safeParse(json);
