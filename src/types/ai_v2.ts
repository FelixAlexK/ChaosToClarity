...
export const taskSchemaV2 = z.object({
	...
	category: z
		.string()
		.describe(`The category of the task, e.g. ${categories.join(", ")}.`),
	priority: z
		.enum(["high", "medium", "low"])
		.describe("The priority level of the task."),
	estimated_time: z
		.string()
		.describe("Estimated time to complete the task, e.g., '30min', '1h'."),
	deadline: z
		.string()
		.nullable()
		.describe("Deadline for the task if recognizable, otherwise null."),
	completed: z
		.boolean()
		.default(false)
...
export const daySchemaV2 = z.object({
	...
	start: z
		.string()
		.describe(
			"Start date and time for the task. If not specified, use the given current date and time.",
		),
	end: z.string().describe("Deadline date and time for the task."),
	completed: z
		.boolean()
		.default(false)
		.describe("Whether the task is completed on this day."),
