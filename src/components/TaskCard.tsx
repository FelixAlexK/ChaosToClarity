...
export function TaskCard({ task, updateTask, deleteTask }: TaskCardsProps) {
	...
	const [color, setColor] = useState(
		task.color ?? (() => getCustomColor(getCategoryHex(task.category), 100))(),
	);
	...
	if (completed && !isEditing) {
		return (
			<TaskCardDelete
				...
				markedAsDone={() => handleMarkTaskAsDone(!completed)}
				task={task}
			/>
		);
	}
		...
		>
			...
			{isEditing && (
				<>
					...
					<CardContent className="space-y-2">
						<Select
							onValueChange={(value) => setNewPriority(value as PriorityV2)}
						>
...
function TaskCardDelete({
	...
	return (
		<Card
			onClick={markedAsDone}
			className="bg-muted-foreground/20 border-muted-foreground/60"
		>
			<CardHeader>
				...
				<CardTitle className="text-xl truncate text-muted-foreground">
					{task.title}
				</CardTitle>
			</CardHeader>
